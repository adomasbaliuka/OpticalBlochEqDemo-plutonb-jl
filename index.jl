...
# ╔═╡ 8a91db1a-03f0-11ee-1958-6da2a99c3d9b
begin
	using QuantumOptics
	using Plots
	using LaTeXStrings
...
# ╔═╡ 9642e591-58fb-448a-b653-e144b5b8d8c9
begin
	# Parameters (defined below as sliders)
	#γ = 2.0
	#η = 1.5
	#Δ = 0.0
	T = [0:0.05:10;]
...
# ╔═╡ b7c0e4d9-4084-4d2c-9104-9adaf6ec5f60
begin
	gamma_slider = @bind γ Slider(0:0.1:8, show_value=true, default = 2.)
	delta_slider = @bind δ Slider(0:0.1:8, show_value=true, default=0.)
	eta_slider = @bind Ω₀ Slider(0:0.1:8, show_value=true, default = 1.5)
	md"""	
	γ: $(gamma_slider)   
	δ: $(delta_slider)   
	Ω₀: $(eta_slider)
...
# ╔═╡ 656749ee-3e6b-4aaa-8f20-94c3b6882ad4
begin
	# Basis and operators
	b1 = SpinBasis(1//2)
	sm = sigmam(b1)  # matrix |ground><excited| 
	sp = sigmap(b1)  # matrix |excited><ground|
	sz = sigmaz(b1)  # pauli matrix σ_z
	
	# Deterministic part of the Hamiltonian (ħ=1)
	H = 0.5*(δ*sz - Ω₀*(sp + sm) )
...
# ╔═╡ 03e32213-b93c-4613-ba9a-091f4ba2f043
# Initial state
ψ0 = spinup(b1)

# ╔═╡ ffe18f08-03d1-4f5a-8c26-18e4d54f0af2
# Average over stochastic Schrödinger equations
fout(t, psi) = (
	real(expect(QuantumOptics.projector(spinup(b1)), psi)), real(expect(QuantumOptics.projector(ψ0), psi)),
)

# ╔═╡ f3e152ed-562b-446c-8fbd-5f8a2adc1654
(collect(H))

# ╔═╡ ce6ffcf6-9e09-4c46-bec4-ed4320107fde
begin
	# Master equation with dephasing
	tout, fout_vals = timeevolution.master(T, ψ0, H, [sz]; rates=[0.25γ], fout=fout)
	occ_spinup = first.(fout_vals)
	occ_spindown = last.(fout_vals)
end

# ╔═╡ 58603de3-be4f-410d-b163-596d4c8e9f44

begin
	plot(tout, occ_spinup, label="", legend=:outerbottom)
	#plot!(tout, occ_spindown, label="down")
	labelplotfun = @latexrun f(t) = (Ω₀ / sqrt(Ω₀^2 + δ^2))^2*cos(sqrt(Ω₀^2 + δ^2) * t / 2)^2 + (δ/sqrt(Ω₀^2 + δ^2))^2
	plot!(tout, f, label=labelplotfun)
	#plot!(tout, z_im)
	# axis([0, T[end], -1, 1])
	yaxis!([0, 1.1])
	yticks!(-1:0.2:1)
...
julia_version = "1.9.0"
manifest_format = "2.0"
project_hash = "9754add410f7b101406bca78205242c59c5387b4"
...
git-tree-sha1 = "50f82e2a705a544af539acc6a03674a4d7eee1d8"
...
[[deps.HalfIntegers]]
git-tree-sha1 = "d654a64bf8861db5fd95204a4955f0f28e791c62"
...
git-tree-sha1 = "cb2b768c0c9fb7bf8cee52b9ac5a6be7fa5f6276"
...
git-tree-sha1 = "c9d0b86b8f1d419c4f6191f7a1fcf3d073ebf223"
